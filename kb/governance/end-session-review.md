...
- **Containment (verified 2026-08-09):** `--allowedTools` is an allow-list, not a restriction — omitted tools fall through to the rest of the permission stack, where read-only shell commands are auto-approved. So the guarantees rest on two mechanisms:
  - **Writes:** only `Edit(path)` rules grant file writes. The run can write inside `kb/governance/amendments/` and `kb/improvements/` and nowhere else — tested directly: a write to an unlisted `kb/` path is denied. It cannot modify the constitution, live skills, hooks, or `CLAUDE.md`.
  ...
  - **Known gap:** `.claude/staging/` is unwritable — `.claude/` is a built-in sensitive path that an `Edit()` rule cannot override — so staged artifacts are currently inlined into proposals instead. Relocation pending.
