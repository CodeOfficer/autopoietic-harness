...
4. **Promote by tier.** Session ledger → consumer repo layer (proposal targets the target repository's `kb/` or `CLAUDE.md`) → global plugin engine baseline (`$CLAUDE_PLUGIN_OPTION_PLUGIN_SOURCE_PATH`). Upstream plugin promotion occurs when the owner interactively selects Upstream Maintainer Mode during `/autopoietic-harness:ratify`. Reviews may also propose deprecation when a mechanism's target friction no longer appears.
