...
7. **Least privilege and containment.** Triggered by any agent task: agents operate with the minimum access the task requires; secrets never enter any `kb/`; irreversible or outward-facing actions require prior owner approval. Any pipeline that moves raw tool telemetry toward a `kb/` destination must summarize, not verbatim-quote, ledger detail fields. A run whose observed tool access exceeds a containment the repository documents for it must record the discrepancy rather than proceed as if the restriction held.
8. **Configuration integrity and zero dead config.** Triggered by any configuration change: `.claude-plugin/plugin.json` `userConfig` is the canonical schema for all plugin configuration parameters. Every declared parameter must be dynamically resolved and consumed by runtime logic. Adding unused parameters or un-documented config options is prohibited. Documentation in `README.md` must mirror `userConfig` exactly.
