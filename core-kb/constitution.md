...
8. **Configuration integrity and zero dead config.** Triggered by any configuration change: every declared configuration parameter (including `userConfig` in `.claude-plugin/plugin.json`, environment variables, `.autopoietic/config.json`, or CLI options) must be actively consumed and enforced by executable hooks, skills, or runtime logic. Declaring unused, phantom, or dead configuration parameters is strictly prohibited.
