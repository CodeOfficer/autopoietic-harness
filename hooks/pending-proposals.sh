...
friction_proposals="$repo_dir/.claude/friction/proposals.json"
...
if [ -n "$list" ]; then
  list="$list, .claude/friction/proposals.json ($json_count item(s))"
else
  list=".claude/friction/proposals.json ($json_count item(s))"
fi
